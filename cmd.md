...
```
Команда для определения местарасположения фала
```sh
git init
```
...
5. (не обязательно) git checkout <указать первую часть уникального шифра> переход к нужной версии (например если нужно откатиться назад) сохренения;
